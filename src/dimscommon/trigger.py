...
def trigger_to_dict(trigger):
    ...
    resulting_dict = {
        ...
        'box_min_x': trigger.bounding_rect.min_x,
        'box_min_y': trigger.bounding_rect.min_y,
        'box_max_x': trigger.bounding_rect.max_x,
        'box_max_y': trigger.bounding_rect.max_y,
    }
...
def get_triggers_by_ids(trigger_ids: List[int],
    ...
    _assert(trigger_ids != [], "Trigger ids cannot be empty")

    cursor.execute(
        f"SELECT * FROM all_triggers WHERE id in ({','.join([str(trigger_id) for trigger_id in trigger_ids])})"
    )
